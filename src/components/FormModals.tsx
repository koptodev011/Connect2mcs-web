...
import formModalStyles from "./FormModals.module.css";
...
export function PostJobModal({
  ...
  async function submit() {
    ...
    if (!token) {
      toast.add(
        isInternship
          ? "Please log in to post an internship"
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          result.error ||
            (isInternship ? "Could not post internship" : "Could not post job"),
        );
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={isInternship ? "Post an Internship" : "Post a Job"}
      width={520}
    >
      <Field
        label={isInternship ? "Internship title" : "Name"}
        value={form.name}
        onChange={set("name")}
        placeholder={
          isInternship ? "e.g. Software Engineering Intern" : "Job post name"
        }
      />
      <Field
        label="Description"
...
// ── Add Mandal Modal ──────────────────────────────────────────────────────────
export function AddMandalModal({
  isOpen,
  onClose,
}: {
  isOpen: boolean;
  ...
  function submit() {
    if (!form.name || !form.city || !form.country) {
      toast.add("Please fill name, city, and country", "error");
      return;
    }
    toast.add(
      "Mandal submitted for review. We'll verify and list it within 3 days.",
      "success",
    );
    onClose();
    setForm({
  ...
  return (
    ...
    >
      <Field
        ...
        value={form.name}
        onChange={set("name")}
        placeholder="Maharashtra Mandal Toronto"
      />
      ...
      <Field
        ...
        onChange={set("desc")}
        multiline
        placeholder="Brief description of the organisation, its activities, and community…"
      />
      <Field
        label="Contact email"
        value={form.contact}
...
export function HostEventModal({
  ...
  function submit() {
    if (!form.title || !form.date || !form.venue) {
      toast.add("Please fill title, date, and venue", "error");
      return;
    }
    toast.add(
      "Event submitted! It will appear in the listing after review.",
      "success",
    );
    onClose();
    setForm({
  ...
  return (
    ...
    >
      <Field
        ...
        value={form.title}
        onChange={set("title")}
        placeholder="Gudhi Padwa Celebration 2026"
      />
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <Field
          label="Date"
      ...
      <Field
        ...
        value={form.venue}
        onChange={set("venue")}
        placeholder="Edison Convention Centre"
      />
      <Field
        label="City"
        value={form.city}
      ...
      <Field
        ...
        onChange={set("desc")}
        multiline
        placeholder="What's happening? Who's it for? Any highlights…"
      />
      <Field
        label="Organiser / Mandal name"
        value={form.organiser}
        onChange={set("organiser")}
        placeholder="BMM New Jersey"
      />
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <Field
        ...
        />
        <Field
          label="External link (RSVP / tickets)"
          value={form.link}
          onChange={set("link")}
...
export function PostListingModal({
  ...
  const handleCountryChange = (value: string) => {
    ...
    const cur = code
      ? currencies.find(
          (c) => (c.ISO_Code || c.C_ISO_Code || "").toUpperCase() === code,
        )
      : undefined;
    setForm((f) => ({
      ...f,
  ...
  useEffect(() => {
    ...
    fetch("/api/v1/models/MCS_MarketPlace_Category", { cache: "no-store" })
      .then((res) =>
        res.ok
          ? res.json()
          : Promise.reject(new Error("Could not load categories")),
      )
      .then((data: { records?: MarketplaceCategory[] }) => {
        const records = Array.isArray(data.records) ? data.records : [];
        setCategories(
          records.filter((c) => c.IsActive !== false && (c.Name || "").trim()),
        );
      })
      .catch((error) => {
        console.error("Could not load marketplace categories:", error);
        setCategories([]);
      });
    fetch("/api/v1/models/C_Currency", { cache: "no-store" })
      ...
      .then((data: { records?: CurrencyRecord[] }) =>
        setCurrencies(Array.isArray(data.records) ? data.records : []),
      )
      .catch((error) => {
        console.error("Could not load currencies:", error);
        setCurrencies([]);
      });
    fetch("/api/data/countries", { cache: "no-store" })
      ...
      .then((data: CountryOption[]) => {
        ...
        if (
          location.countryId &&
          list.some((c) => String(c.id) === String(location.countryId))
        ) {
          setForm((f) =>
            f.countryId ? f : { ...f, countryId: String(location.countryId) },
          );
        }
      })
      .catch((error) => {
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/v1/models/C_City?countryId=${encodeURIComponent(form.countryId)}`,
  ...
  const categoryNames =
    categories.length > 0
      ...
      : FALLBACK_CATEGORIES;
  const categoryId =
    categories.find((c) => (c.Name || "").trim() === form.cat)?.id ?? 0;
  ...
  const currencyCode =
    CURRENCY_BY_REGION[String(selectedCountry?.code || "").toUpperCase()];
  const derivedCurrency = currencyCode
    ? currencies.find(
        (c) =>
          (c.ISO_Code || c.C_ISO_Code || "").toUpperCase() === currencyCode,
      )
    : currencies[0];
  ...
  async function submit() {
    if (!form.title || !form.price) {
      toast.add("Please fill in title and price", "error");
      return;
    }
    if (!form.countryId || !form.cityId) {
      toast.add("Please select a country and city", "error");
      return;
    }
    let userId = "";
    try {
      userId = String(
        JSON.parse(localStorage.getItem("mcs_user") || "{}").id || "",
      );
    } catch {}
    if (!userId) {
      toast.add("Please sign in before posting a listing.", "error");
      return;
    }
    if (imageFile) {
    ...
    try {
      ...
      const locationValue = [selectedCity?.name, selectedCountry?.name]
        .filter(Boolean)
        .join(", ");
      const now = new Date();
      ...
      const response = await fetch("/api/v1/models/MCS_MarketPlaces", {
        ...
        body: JSON.stringify({
          ...
          MCS_MarketPlace_Category_ID: {
            id: Number(categoryId) || 0,
            identifier: form.cat,
          },
          MCS_PostedBy_ID: { id: Number(userId) || 0 },
          MCS_Logo_ID: logoData
            ? { data: logoData, name: imageFile?.name || "Listing image" }
            : { id: 0 },
          Value: form.title,
          Name: form.title,
          Description:
            form.desc.trim() || "Marketplace listing created from Postman",
      ...
      if (!response.ok)
        throw new Error(result.error || "Could not post listing");
      ...
      toast.add(
        error instanceof Error ? error.message : "Could not post listing.",
        "error",
      );
    } finally {
      setSubmitting(false);
  ...
  return (
    ...
    >
      <Field
        ...
        value={form.title}
        onChange={set("title")}
        placeholder="MacBook Pro 2022, Kadhai set…"
      />
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <Field
          label="Price"
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        <label style={{ display: "block", marginBottom: 16 }}>
          ...
          >
            <option value="">
              {form.countryId ? "Select city" : "Select country first"}
            </option>
            {cities.map((c) => (
              <option key={String(c.id)} value={String(c.id)}>
                {c.name}
      ...
      <label style={{ display: "block", marginBottom: 16 }}>
        ...
        >
          ...
          {currencies.length > 0 ? (
            currencies.map((c) => (
              <option key={String(c.id)} value={String(c.id)}>
                {c.ISO_Code || c.C_ISO_Code || c.Name || c.id}
              </option>
            ))
          ) : (
      ...
      <Field
        ...
        onChange={set("desc")}
        multiline
        placeholder="Describe the item, any defects, reason for selling…"
      />
      <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
        <Btn
          kind="primary"
...
export function ContactModal({
  ...
  function submit() {
    if (!msg.trim()) {
      toast.add("Please write a message", "error");
      return;
    }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title} width={480}>
      ...
      <Field
        ...
        onChange={setMsg}
        multiline
        placeholder="Hi, I'm interested. Is this still available?"
      />
      <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
        <Btn kind="primary" size="md" full onClick={submit}>
          Send message
...
export function ListBusinessModal({
  ...
  useEffect(() => {
    ...
    Promise.resolve().then(() => {
      setForm({
        ...
        owner: business?.owner || savedUser.name || "",
        desc: business?.desc || "",
        countryId: business?.countryId || "",
        cityId: business?.cityId || "",
        phone: business?.phone || savedUser.phone || "",
    ...
    fetch("/api/v1/models/MCS_Business_Category", { cache: "no-store" })
      ...
      .then((data: { records?: BusinessCategoryOption[] }) =>
        setCategories(
    ...
    if (savedUser.id && token) {
      fetch(
        `/api/v1/models/ad_user/${encodeURIComponent(String(savedUser.id))}`,
        {
        ...
        .then((response) =>
          response.ok
            ? response.json()
            : Promise.reject(new Error("Could not load user")),
        )
        .then((user: { Name?: string; Phone?: string; Mobile?: string }) => {
          setForm((current) => ({
            ...current,
            owner: business?.owner || user.Name || savedUser.name || "",
            phone:
              business?.phone ||
              user.Phone ||
              user.Mobile ||
        ...
        .catch((error) =>
          console.error("Business owner prefill failed:", error),
        );
    }
  }, [business, isOpen]);
  ...
  function handleCountryChange(countryId: string) {
    setForm((current) => ({ ...current, countryId, cityId: "" }));
  }
  ...
  async function submit() {
    if (
      ...
      toast.add(
        "Please fill business name and owner, then select category, country, and city",
        "error",
      );
      return;
    }
    ...
    try {
      const payload = {
        ...
        C_City_ID: { id: form.cityId },
        C_Country_ID: { id: form.countryId },
        MCS_Business_Category_ID: { id: form.categoryId },
        Value: business?.id ? business.name : `MCS-BUS-${Date.now()}`,
        Name: form.name.trim(),
        Description: form.desc.trim(),
        Help: form.desc.trim(),
        MCS_Services: form.services.trim(),
        MCS_siteUrl: form.website.trim(),
      ...
      toast.add(
        business
          ...
          : "Business submitted! We'll verify and list it within 48h.",
        "success",
      );
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={business ? "Edit Your Business" : "List Your Business"}
      marathi="व्यवसाय नोंदवा"
      width={560}
    >
      <Field
        label="Business name"
        value={form.name}
        onChange={set("name")}
        placeholder="Sharma Law Associates"
      />
      <div className={formModalStyles.twoColumns}>
        <label className={formModalStyles.selectField}>
          <span className={formModalStyles.selectLabel}>Category</span>
          <select
      ...
      <div className={formModalStyles.twoColumns}>
        <label className={formModalStyles.selectField}>
          <span className={formModalStyles.selectLabel}>Country</span>
          <select
            value={form.countryId}
        ...
        <label className={formModalStyles.selectField}>
          <span className={formModalStyles.selectLabel}>City</span>
          <select
            value={form.cityId}
      ...
      <Field
        ...
        placeholder="Immigration law, visa, green card…"
      />
      <Field
      ...
      <div className={formModalStyles.actions}>
        <Btn
          kind="primary"
          size="md"
          full
  ...
  async function submit() {
    ...
    try {
      const newGroup = {
        ...
        members: 1,
        posts: "1 post today",
        tone: ["saffron", "brick", "green", "blue", "gold"][
  ...
  return (
    ...
    >
      <Field
        ...
        value={name}
        onChange={setName}
        placeholder="Pune Alumni Network"
      />
      <Field
        label="Description"
        value={desc}
      ...
      <Field
        ...
        value={cat}
        onChange={setCat}
        placeholder="Alumni, Culture, Professional, Family…"
      />
...
export function BecomeMentorModal({
  ...
  return (
    <BookModal
      ...
      marathi="मार्गदर्शक व्हा"
      submitLabel="Apply to mentor"
      fields={[
        {
          key: "expertise",
          label: "Areas of expertise",
          placeholder: "Engineering, Career transitions, Immigration...",
        },
        {
          key: "role",
          label: "Current role",
        ...
        },
        {
          ...
          placeholder: "Tech, Design, Medicine…",
        },
        {
          key: "avail",
          label: "Availability",
          placeholder: "Weekends, 1h/week",
        },
        {
          key: "bio",
          label: "Bio / Why you want to mentor",
          placeholder: "",
          multiline: true,
        },
      ]}
...
export function BookSessionModal({
  ...
  function submit() {
    if (!slot) {
      toast.add("Please select a time slot", "error");
      return;
    }
    toast.add(
      `Session booked with ${mentorName || "mentor"} on ${slot}!`,
      "success",
    );
    setSlot("");
    setMsg("");
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Book session${mentorName ? ` with ${mentorName}` : ""}`}
      width={460}
    >
          ...
          {slots.map((s) => (
            <button
              ...
              style={{
                padding: "12px 16px",
                borderRadius: 10,
                border: `1.5px solid ${slot === s ? C.saffron : C.line}`,
                background: slot === s ? C.saffronLt : "#fff",
                cursor: "pointer",
                textAlign: "left",
                fontSize: 14,
                fontWeight: slot === s ? 700 : 500,
                color: slot === s ? C.saffronDk : C.ink,
                fontFamily: "inherit",
  ...
  return (
    <BookModal
      ...
      fields={[
        ...
        {
          key: "threshold",
          label: "Alert me when rate reaches",
          placeholder: "86.50",
        },
        {
          key: "email",
  ...
  return (
    <BookModal
      ...
      fields={[
        ...
        {
          key: "cats",
          label: "Offer categories (optional)",
          placeholder: "Food, Travel, Finance…",
        },
      ]}
    />
  );
    ...
    <BookModal
      ...
      fields={[
        { key: "biz", label: "Business name", placeholder: "Your company" },
        {
          ...
          multiline: true,
          placeholder: "What offer or deal do you want to feature?",
        },
      ]}
    />
  );
...
// ── Post Housing Modal ────────────────────────────────────────────────────────
export function PostHousingModal({
  isOpen,
  onClose,
}: {
  isOpen: boolean;
      ...
      fields={[
        {
          key: "title",
          label: "Listing title",
          placeholder: "2BR near Harvard, roommate wanted",
        },
        ...
        { key: "rent", label: "Monthly rent", placeholder: "$1,200 / mo" },
        {
          key: "type",
          label: "Type",
          placeholder: "Roommate / Whole place / Short stay",
        },
        {
          key: "desc",
          label: "Description",
          multiline: true,
          placeholder: "Furnishing, rules, nearby transit, move-in date…",
        },
      ]}
    />
  );
...
export function BecomeProviderModal({
  isOpen,
  onClose,
}: {
  isOpen: boolean;
  ...
  return (
    <BookModal
      isOpen={isOpen}
      onClose={onClose}
      title="Become a Tiffin Provider"
      marathi="डबा पुरवठादार"
      submitLabel="Apply"
      fields={[
        { key: "city", label: "City", placeholder: "Boston, MA" },
        {
        ...
        },
        {
          ...
          placeholder: "20 tiffins/day",
        },
        {
          key: "price",
          label: "Price per meal",
          placeholder: "$12/meal or $250/month",
        },
      ]}
    />
  );
...
export function TiffinSubscribeModal({
  isOpen,
  onClose,
  providerName,
  basePrice,
  ...
  const plans = [
    {
      ...
      label: "Trial Box",
      price: basePrice,
      desc: "One-time box to taste the food before committing.",
    },
    {
      id: "weekly",
      label: "Weekly",
      price: basePrice,
      desc: "5 days of meals. Cancel anytime week-to-week.",
    },
    {
      id: "monthly",
      label: "Monthly",
      price: basePrice,
      desc: "20 days of meals. Best value, save 10%.",
    },
  ];
  ...
  function submit() {
    if (!plan) {
      toast.add("Please select a plan", "error");
      return;
    }
    if (!address.trim()) {
      toast.add("Please enter your delivery address", "error");
      return;
    }
    toast.add(
      `Subscribed to ${providerName} (${plans.find((p) => p.id === plan)?.label}, ${meals} meal${meals > 1 ? "s" : ""}/day)!`,
      "success",
    );
    setPlan("");
    setMeals(1);
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Subscribe to ${providerName}`}
      width={480}
    >
      {/* Plan selection */}
      <div style={{ marginBottom: 16 }}>
          ...
          {plans.map((p) => (
            <button
              ...
              style={{
                padding: "12px 14px",
                borderRadius: 10,
                border: `1.5px solid ${plan === p.id ? C.saffron : C.line}`,
                background: plan === p.id ? C.saffronLt : "#fff",
                cursor: "pointer",
                textAlign: "left",
                fontFamily: "inherit",
                transition: "all 0.12s",
            ...
            >
              ...
              >
                <span
                  style={{
                    fontSize: 14,
                    fontWeight: plan === p.id ? 700 : 600,
                    color: plan === p.id ? C.saffronDk : C.ink,
                  }}
                >
                  {p.label}
                </span>
                <span
                  style={{
                    fontSize: 13,
                    fontWeight: 700,
                    color: plan === p.id ? C.saffronDk : C.ink2,
                  }}
                >
                  {p.price}
                </span>
          ...
          {[1, 2, 3].map((n) => (
            <button
              ...
              style={{
                ...
                padding: "10px 0",
                borderRadius: 10,
                border: `1.5px solid ${meals === n ? C.saffron : C.line}`,
                background: meals === n ? C.saffronLt : "#fff",
                cursor: "pointer",
                fontFamily: "inherit",
                fontSize: 14,
                fontWeight: meals === n ? 700 : 600,
                color: meals === n ? C.saffronDk : C.ink,
                transition: "all 0.12s",
              }}
            >
              {n} meal{n > 1 ? "s" : ""}
            ...
            return (
              <button
                ...
                style={{
                  padding: "8px 12px",
                  borderRadius: 8,
                  border: `1.5px solid ${active ? C.saffron : C.line}`,
                  background: active ? C.saffronLt : "#fff",
                  cursor: "pointer",
                  fontFamily: "inherit",
                  fontSize: 12.5,
                  fontWeight: active ? 700 : 500,
                  color: active ? C.saffronDk : C.ink,
                  transition: "all 0.12s",
                  display: "flex",
                  gap: 6,
                  alignItems: "center",
      ...
      <Field
        ...
        onChange={setAddress}
        multiline
        placeholder="Full delivery address with apartment number, zip code..."
      />
...
export function NewChatModal({
  ...
  function start() {
    if (!selected) {
      toast.add("Please select a person", "error");
      return;
    }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="New Chat" width={400}>
        ...
        {filtered.map((p) => (
          <button
            ...
            style={{
              ...
              padding: "10px 14px",
              borderRadius: 10,
              border: `1.5px solid ${selected === p ? C.saffron : C.line}`,
              background: selected === p ? C.saffronLt : "#fff",
              cursor: "pointer",
              textAlign: "left",
              fontFamily: "inherit",
              transition: "all 0.12s",
          ...
          >
            ...
            <span
              style={{
                ...
                color: selected === p ? C.saffronDk : C.ink,
              }}
            >
              {p}
            </span>
      ...
      <div style={{ display: "flex", gap: 10 }}>
        <Btn kind="primary" size="md" full onClick={start}>
          Start chat
        </Btn>
        <Btn kind="ghost" size="md" onClick={onClose}>
          Cancel
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Btn kind="primary" size="lg" onClick={onClose}>
            Got it
          </Btn>
        </div>
      </div>
    ...
    <BookModal
      ...
      fields={[
        { key: "city", label: "City", placeholder: "Boston, MA" },
        {
          ...
          placeholder: "Shared, Private room, Entire place",
        },
...
export function ApplyModal({
  ...
  onClose: () => void;
  itemName: string;
  onSubmit: (data: { sop: string; link: string }) => void;
}) {
  const toast = useGlobalToast();
  ...
  function submit() {
    if (!form.sop) {
      toast.add("Please provide a Statement of Purpose", "error");
      return;
    }
    onSubmit(form);
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Apply for ${itemName}`}
      width={560}
      marathi="अर्ज करा"
    >
      <Field
        label="Statement of Purpose / Cover Letter"
        value={form.sop}
        onChange={set("sop")}
        multiline
        placeholder="Why are you a good fit? What are your goals? (Min 100 words)"
      />
      <Field
